...
export function Input({ class: className, ...props }: InputProps) {
  return (
    <input
      class={`rounded-lg bg-page border border-edge px-3 py-2 text-sm text-content placeholder:text-content-faint shadow-xs w-full focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/15 transition-colors duration-100 disabled:opacity-50 disabled:cursor-not-allowed ${className ?? ""}`.trim()}
      {...props}
    />
  );
}
