...
export const TogglesModel = createModel(() => {
	...
	const fetch = async (projectId: string) => {
		...
		try {
			...
			error.value =
				err instanceof Error ? err.message : "Failed to load toggles";
		} finally {
			loading.value = false;
		}
	};
	...
	const toggle = async (projectId: string, id: string, enabled: boolean) => {
		...
		try {
			...
			toggles.value = prev;
			error.value =
				err instanceof Error ? err.message : "Failed to update toggle";
		}
	};
	...
	const remove = async (projectId: string, id: string) => {
		...
		try {
			...
			toggles.value = prev;
			error.value =
				err instanceof Error ? err.message : "Failed to delete toggle";
		}
	};
